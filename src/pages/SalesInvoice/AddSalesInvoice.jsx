...
import { Trash2, Plus, Save, Printer, FileText, Calculator, Tag, ArrowLeft } from "lucide-react"
...
const AddSalesInvoice = () => {
  ...
  const [items, setItems] = useState([
    { id: Date.now(), item: '', altQty: 0, altUnit: 0, qty: 0, unit: 0, price: 0, discountPercent: 0, basicAmt: 0, itemTerm: 0, netAmt: 0 }
  ])
  ...
  // --- 3. FETCH DATA ON EDIT MODE ---
  useEffect(() => {
    if (isEditMode) {
      const existing = JSON.parse(localStorage.getItem("salesInvoices")) || []
      // Use String comparison to handle both numerical and string IDs from URL
      const invoiceToEdit = existing.find(inv => String(inv.id) === String(id))
      
      if (invoiceToEdit) {
        setItems(invoiceToEdit.items || [])
        setVoucherTerms(invoiceToEdit.voucherTerms || [])
        if (invoiceToEdit.headerData) {
          setHeaderData(invoiceToEdit.headerData)
        }
      } else {
  ...
  const updateItemField = (rowId, field, value) => {
    setItems(items.map(item => item.id === rowId ? calculateRow({ ...item, [field]: value }) : item));
  }
  ...
  const handleSaveInvoice = () => {
    ...
    if (!headerData.voucherNo || !headerData.customer || !headerData.refChallan || !headerData.refOrder) {
      // alert("Please fill in the Voucher Number and select a Customer.");
      toast.warning("Voucher Number, Customer, RefChallan and RefOrder are mandatory.")
      return; 
    }
    ...
    let updatedList;
    if (isEditMode) {
      updatedList = existing.map(inv => String(inv.id) === String(id) ? invoice : inv)
    } else {
      updatedList = [...existing, invoice]
    }

    localStorage.setItem("salesInvoices", JSON.stringify(updatedList))
    ...
    <div className="p-6 space-y-4 bg-[#F1F5F9] min-h-screen font-sans text-slate-900">
      ...
      <div className="flex justify-between items-center bg-[#4e5d70] p-4 rounded-t-lg border-b-2 border-blue-200">
        <div className="flex items-center gap-3">
          <Button variant="ghost" size="sm" className="text-white hover:bg-slate-600 h-8" onClick={() => navigate("/sales-invoice")}>
             <ArrowLeft className="w-4 h-4 mr-2" />
          </Button>
          <FileText className="text-blue-400 w-5 h-5" />
          <h1 className="text-md font-bold tracking-widest text-white uppercase">
            {isEditMode ? "Edit Sales Invoice" : "Sales Invoice Entry"}
          </h1>
        </div>
        <div className="flex gap-2">
          ...
          <Button onClick={handleSaveInvoice} size="sm" className={`${isEditMode ? 'bg-orange-600 hover:bg-orange-700' : 'bg-blue-600 hover:bg-blue-700'} h-8 px-5 text-[10px] font-black shadow-lg`}>
            <Save className="w-3 h-3 mr-2 text-white"/> 
            <span className='text-white text-xs'>{isEditMode ? "UPDATE CHANGES" : "SAVE INVOICE"}</span>
          </Button>
        </div>
      </div>
      ...
      <Card className="rounded-none border-slate-300 shadow-sm bg-white">
        <CardContent className="p-4 grid grid-cols-6 gap-3">
          ...
          <div className="space-y-1">
            <Label className="text-[10px] font-black text-slate-500 uppercase">Voucher No</Label>
            <Input required className="h-8 text-xs bg-slate-50 border-slate-200" value={headerData.voucherNo}  onChange={(e) => updateHeaderField('voucherNo', e.target.value)} />
          </div>

          {/* Date */}
          <div className="space-y-1">
            <Label className="text-[10px] font-black text-slate-500 uppercase">Date</Label>
            <input type="date" className="h-8 w-full text-xs bg-slate-50 border border-slate-200 rounded px-2" value={headerData.date} onChange={(e) => updateHeaderField('date', e.target.value)} />
          </div>

          {/* Ref Order */}
          <div className="space-y-1">
            <Label className="text-[10px] font-black text-slate-500 uppercase">Ref Order#</Label>
            <Input className="h-8 text-xs bg-slate-50 border-slate-200" value={headerData.refOrder} onChange={(e) => updateHeaderField('refOrder', e.target.value)} />
          </div>

          {/* Ref Challan */}
          <div className="space-y-1">
            <Label className="text-[10px] font-black text-slate-500 uppercase">Ref Challan#</Label>
            <Input className="h-8 text-xs bg-slate-50 border-slate-200" value={headerData.refChallan} onChange={(e) => updateHeaderField('refChallan', e.target.value)} />
          </div>

          {/* Customer */}
          <div className="space-y-1">
            <Label className="text-[10px] font-black text-slate-500 uppercase">Customer</Label>
            <select className="h-8 text-xs w-full bg-slate-50 border border-slate-200 rounded px-2" value={headerData.customer} onChange={(e) => updateHeaderField('customer', e.target.value)}>
              <option value="">Select Customer</option>
              <option value="Sita Rai">Sita Rai</option>
              <option value="Ravi Sharma">Ravi Sharma</option>
              <option value="Anita KC">Anita KC</option>
            </select>
          </div>

          {/* Agent */}
          <div className="space-y-1">
            <Label className="text-[10px] font-black text-slate-500 uppercase">Agent</Label>
            <Input className="h-8 text-xs bg-slate-50 border-slate-200" value={headerData.agent} onChange={(e) => updateHeaderField('agent', e.target.value)} />
          </div>

          {/* Due Dates */}
          <div className="space-y-1">
            <Label className="text-[10px] font-black text-slate-500 uppercase">Due Dates</Label>
            <Input className="h-8 text-xs bg-slate-50 border-slate-200" value={headerData.dueDates} onChange={(e) => updateHeaderField('dueDates', e.target.value)} />
          </div>

          {/* Second Date Field */}
          <div className="space-y-1">
            <Label className="text-[10px] font-black text-slate-500 uppercase">Date</Label>
            <input type="date" className="h-8 w-full text-xs bg-slate-50 border border-slate-200 rounded px-2" value={headerData.date2} onChange={(e) => updateHeaderField('date2', e.target.value)} />
          </div>

          {/* Sub Ledger */}
          <div className="space-y-1">
            <Label className="text-[10px] font-black text-slate-500 uppercase">Sub Ledger</Label>
            <Input className="h-8 text-xs bg-slate-50 border-slate-200" value={headerData.subLedger} onChange={(e) => updateHeaderField('subLedger', e.target.value)} />
          </div>

          {/* Class */}
          <div className="space-y-1">
            <Label className="text-[10px] font-black text-slate-500 uppercase">Class</Label>
            <Input className="h-8 text-xs bg-slate-50 border-slate-200" value={headerData.class} onChange={(e) => updateHeaderField('class', e.target.value)} />
          </div>

          {/* Currency */}
          <div className="space-y-1">
            <Label className="text-[10px] font-black text-slate-500 uppercase">Currency</Label>
            <Input className="h-8 text-xs bg-slate-50 border-slate-200 font-bold" value={headerData.currency} onChange={(e) => updateHeaderField('currency', e.target.value)} />
          </div>

          {/* Rate */}
          <div className="space-y-1">
            <Label className="text-[10px] font-black text-slate-500 uppercase">Rate</Label>
            <Input className="h-8 text-xs bg-slate-50 border-slate-200" value={headerData.rate} onChange={(e) => updateHeaderField('rate', e.target.value)} />
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="border-slate-300 shadow-sm rounded-sm overflow-hidden bg-white">
        <Table>
          <TableHeader className="bg-[#4e5d70]">
            <TableRow className="hover:bg-transparent">
              ...
              <TableHead className="text-[10px] font-bold text-white border-l border-slate-500 uppercase text-center">Qty</TableHead>
              {/* <TableHead className="text-[10px] font-bold text-white border-l border-slate-500 uppercase text-center">Unit</TableHead> */}
          ...
          <TableBody>
            {items.map((item, index) => (
              <TableRow key={item.id} className="border-b border-slate-200 hover:bg-slate-50 transition-colors">
                ...
                <TableCell className="p-1 border-l border-slate-100 bg-blue-50/30">
                  <Input type="number" className="h-7 text-xs text-center border-none font-bold" value={item.qty} onChange={(e) => updateItemField(item.id, 'qty', e.target.value)} />
                </TableCell>
                {/* <TableCell className="p-1 border-l border-slate-100 ">
                  <Input type="text" className="h-7 text-xs text-center border-none" value={item.unit} onChange={(e) => updateItemField(item.id, 'unit', e.target.value)} placeholder="PCS"/>
                </TableCell> */}
        ...
        <div className="p-2 border-t border-slate-300 bg-slate-100">
          <Button variant="outline" size="sm" className="h-7 text-[10px] font-black bg-white border-slate-400" onClick={() => setItems([...items, { id: Date.now(), item: '', qty: 0, price: 0, discountPercent: 0, basicAmt: 0, netAmt: 0 }])}>
            <Plus className="w-3 h-3 mr-1"/> ADD NEW ITEM ROW 
          </Button>
        </div>
      </Card>
      ...
      <div className="grid grid-cols-2 gap-6 mt-2 items-start">
        <div className="space-y-2 bg-white p-4 border border-slate-300 rounded shadow-sm">
          ...
          <Textarea 
            className="min-h-20 text-xs border-slate-200 bg-slate-50 focus:bg-white" 
            value={headerData.remarks}
            onChange={(e) => updateHeaderField('remarks', e.target.value)}
          />
          <div className="flex justify-between text-[10px] font-bold text-slate-500 pt-2">
            <span>Stock Qty : 0.00</span>
            <span>Total Qty : {items.reduce((sum, i) => sum + Number(i.qty), 0)}</span>
          </div>
        </div>
      ...
      <Dialog open={isPriceModalOpen} onOpenChange={setIsPriceModalOpen}>
        <DialogContent className="sm:max-w-87.5 p-0 border-none overflow-hidden rounded-lg shadow-2xl">
          ...
          <div className="p-6 bg-slate-50 space-y-4">
            <div className="space-y-2">
              ...
              <Input type="number" className="font-bold" value={tempPriceData.price} onChange={(e) => setTempPriceData({...tempPriceData, price: e.target.value})} />
            </div>
            <div className="space-y-2">
              <Label className="text-[10px] font-black uppercase text-slate-500">Row Discount (%)</Label>
              <Input type="number" className="font-bold text-red-600" value={tempPriceData.discountPercent} onChange={(e) => setTempPriceData({...tempPriceData, discountPercent: e.target.value})} />
            </div>
            <Button onClick={applyPriceData} className="w-full bg-blue-600 hover:bg-blue-700 font-black uppercase text-xs h-10 mt-2 text-white">
              Update Row Amount
            </Button>
          </div>
        </DialogContent>
      ...
      <Dialog open={isVoucherModalOpen} onOpenChange={setIsVoucherModalOpen}>
        <DialogContent className="sm:max-w-md p-0 overflow-hidden border-none rounded-lg">
          <DialogHeader className="bg-[#1E293B] p-4 text-white">
